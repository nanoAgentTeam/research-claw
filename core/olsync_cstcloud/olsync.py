...
from core.olsync_cstcloud.olclient import OverleafClient, DEFAULT_BASE_URL, DEFAULT_LOGIN_PATH, DEFAULT_PROJECT_PATH, DEFAULT_COOKIE_NAMES
import core.olsync_cstcloud.olbrowserlogin as olbrowserlogin
...
def build_instance_config(base_url=None, login_path=DEFAULT_LOGIN_PATH, project_path=DEFAULT_PROJECT_PATH,
                          cookie_names=None):
    return {
        "base_url": normalize_base_url(base_url or DEFAULT_BASE_URL),
        "login_path": login_path,
        "project_path": project_path,
        "cookie_names": list(cookie_names or DEFAULT_COOKIE_NAMES),
    }


def load_store(cookie_path):
    with open(cookie_path, 'rb') as f:
        store = pickle.load(f)

    # Backward compatible with existing .olauth format.
    store.setdefault("instance", build_instance_config())
    return store


def assert_instance_compatible(store, base_url_override):
    if not base_url_override:
        return

    persisted_base_url = normalize_base_url(store.get("instance", {}).get("base_url", DEFAULT_BASE_URL))
    requested_base_url = normalize_base_url(base_url_override)

    if persisted_base_url != requested_base_url:
        raise click.ClickException(
            "Stored authentication belongs to `{}` but requested base URL is `{}`. "
            "Please run `ols login --path <auth-file> --base-url {}` first.".format(
                persisted_base_url,
                requested_base_url,
                requested_base_url,
            )
...
def build_client_from_store(store):
    instance = store.get("instance", build_instance_config())
    return OverleafClient(
        store["cookie"],
        store["csrf"],
        base_url=instance["base_url"],
        login_path=instance["login_path"],
        project_path=instance["project_path"],
        cookie_names=instance["cookie_names"],
    )
...
@click.option('--base-url', 'base_url', default=None, type=str,
              help="Override Overleaf base URL for this command. Must match .olauth if provided.")
@click.version_option(package_name='overleaf-sync')
@click.pass_context
def main(ctx, local, remote, project_name, cookie_path, sync_path, olignore_path, verbose, base_url):
    if ctx.invoked_subcommand is None:
        if not os.path.isfile(cookie_path):
            raise click.ClickException(
                "Persisted Overleaf cookie not found. Please login or check store path.")

        store = load_store(cookie_path)
        assert_instance_compatible(store, base_url)

        overleaf_client = build_client_from_store(store)
        ...
        def ensure_project_infos():
            ...
            if project_infos is None:
                project_infos = execute_action(
                    ...
                    "Project details could not be queried. This instance may block Socket.IO joinProject. "
                    "Remote-to-local sync is still available, but local upload/delete requires project details.",
                    verbose)
        ...
        # Read local inclusion list once to avoid repeated notices from olignore_keep_list.
        local_keep_files = olignore_keep_list(olignore_path)
        ...
        if remote or sync:
            sync_func(
                ...
                deleted_files=[f for f in local_keep_files if f not in zip_file.namelist() and not sync],
        ...
        if local or sync:
            sync_func(
                files_from=local_keep_files,
                deleted_files=[f for f in zip_file.namelist() if f not in local_keep_files and not sync],
                create_file_at_to=lambda name: overleaf_client.upload_file(
                    project["id"], ensure_project_infos(), name, os.path.getsize(name), open(name, 'rb')),
                delete_file_at_to=lambda name: overleaf_client.delete_file(project["id"], ensure_project_infos(), name),
...
@click.option('-v', '--verbose', 'verbose', is_flag=True, help="Enable extended error logging.")
@click.option('--base-url', 'base_url', default=DEFAULT_BASE_URL, type=str,
              help="Overleaf base URL, e.g. https://sharelatex-lin.cstcloud.cn")
@click.option('--login-path', 'login_path', default=DEFAULT_LOGIN_PATH, type=str,
              help="Login path for interactive browser auth.")
def login(cookie_path, verbose, base_url, login_path):
    if os.path.isfile(cookie_path) and not click.confirm(
            'Persisted Overleaf cookie already exist. Do you want to override it?'):
        return

    instance_config = build_instance_config(base_url=base_url, login_path=login_path)
    click.clear()
    execute_action(lambda: login_handler(cookie_path, instance_config), "Login",
...
@click.option('--base-url', 'base_url', default=None, type=str,
              help="Override Overleaf base URL for this command. Must match .olauth if provided.")
def list_projects(cookie_path, verbose, base_url):
    def query_projects():
...
@click.option('--base-url', 'base_url', default=None, type=str,
              help="Override Overleaf base URL for this command. Must match .olauth if provided.")
def download_pdf(project_name, download_path, cookie_path, verbose, base_url):
...
def login_handler(path, instance_config):
    store = olbrowserlogin.login(
        base_url=instance_config["base_url"],
        login_path=instance_config["login_path"],
        project_path=instance_config["project_path"],
        cookie_names=instance_config["cookie_names"],
    )
    if store is None:
        return False
    store["instance"] = instance_config

    ...
    return [
        AutomationJob(
            ...
            prompt=(
                ...
                "5) 推送策略（必推型）：只要检索到去重后的新论文，无论相关度高低都必须推送，不做过滤；"
                "若当日搜索结果去重后为空，则不推送。\n"
                "6) 推送格式（重要：推送内容必须是完整报告，用户无法看到对话，推送是唯一渠道）：\n"
                "   每篇论文包含以下全部信息：\n"
                "   - 标题（原文）\n"
                "   - 完整链接（用户可直接点击访问的 URL，如 https://arxiv.org/abs/xxxx.xxxxx 或 https://doi.org/xxx）。"
                "不要只写 arXiv ID 或 DOI 编号，必须给出完整可点击的 URL\n"
                "   - 作者列表\n"
                "   - 3-5 句摘要：核心贡献、方法要点、关键结果\n"
                "   - 与本项目的关系标签（方向一致 / 思路启发 / 方法可借鉴 / 方向可延续 / 潜在竞争 / 仅供了解）\n"
                "   - 对本项目的具体启示或可借鉴之处（1-2 句）\n"
                "   禁止只发摘要式短句或「详情见对话框」——用户看不到对话。"
                + _MEMORY_GUIDANCE
            ),
            enabled=True,
            managed_by="system",
        ...
        AutomationJob(
            ...
            prompt=(
                ...
                "4) 推送策略（必推型）：只要本周有任何新内容（论文、预警、日期更新等），不论重要程度都必须推送；"
                "每条内容需标注与项目的关系（方向一致/思路启发/方法可借鉴/方向可延续/潜在竞争/仅供了解）；"
                "若所有任务本周完全无产出，才跳过推送。\n"
                "   重要：推送内容必须是完整周报，不得只发标题列表或摘要。"
                "用户无法看到对话，推送是唯一渠道。禁止写「详情见对话框」。"
                + _MEMORY_GUIDANCE
            ),
            enabled=True,
            managed_by="system",
